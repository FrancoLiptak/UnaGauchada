...
<head>
  ...
  Primero, estamos usando pop ups (mas precisamente divs de la clase modal) y se complico un poco el pasaje de parametros a estos. Lo que hago es que, al hacer click en el boton que abre el modal para editar/borrar la cate particular, disparar una funcion js (que esta en este archivo abajo de todo) que lo que hace es asignar el data-id del boton presionado (que va a ser la concatenacion: id/nombre de la categoria a la que esta asociado el boton) al elemento html dentro del body del modal que tenga el id que yo le digo en la funcion (ver abajo).
  NOTAS:
    -Es importante notar los botones para elimnar y editar deben tener el att data-id="<datos de la cate>".
    -Como en la funcion js lo que hago es cambiar el value de un elemento dentro del modal-body, ese elemento debe ser un input, li, option entre otros; es decir, algun elemento html que tenga el att value. De todos los elementos que tienen value me sirve el input, es por eso que el nombre de la categoria se ve en un input text disabled, y necesito crear un form para que se envie el id de la categoria en un input hidden idCategory al apretar 'Si, acepto'. ESTO ES IMPORTANTE PORQUE RECIEN ACA ES CUANDO SE MANDA A BORRAR!!!!! Se procesaria ese form en un eliminarCate.php.
  */
