...
function showComment($comment, $idGauchada, $numComment, $isReply = false)
{
    ...
    <div 
        ...
        ?>
    >
            <div class='col-md-12' style="margin-bottom: -20px; <?php if($isReply){ ?>  background-color: #f5f5f5;  margin-left: 75px; <?php } ?>">
                ...
                <div class="col-md-4" style="float: right; margin-right: 0px; padding-right: 0px;" >
                    ...
                    if ( validateLogin() && ( isAdmin() || $_SESSION['idUsers'] == $idUserGauchada || $_SESSION['idUsers'] == $comment['idUser']) ){
                        ?> 
                        <div class="col—md—6">
                            <a href="" class="btn btn-danger" style="<?php if ($isReply) echo "margin-left: -75px;"; ?>padding-right: 18px; padding-left: 18px;" onclick="">Eliminar </a>
                        </div>
                    <?php }
                    ?>
           ...
           <?php 
           if ($reply->num_rows > 0) {?>
                    <p style="margin-left:110px; margin-bottom: 0px; margin-top: 85px;"><span class='badge'>1</span> Respuesta para <?php echo  $userName ?>:</p><br>
                    <?php 
